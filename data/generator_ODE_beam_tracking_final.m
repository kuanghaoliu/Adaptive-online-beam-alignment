...
% file number: 10 files for testing, \tau = {0.01, 0.02, ..., 0.99}
file_num = 10;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
MM_data = zeros(file_size, 2, 4001, MM_narrow_beam_num); %%%%%%%%%%%%%
% MM optimal beam index
beam_label = zeros(file_size, 4001);   %%%%%%%%%%%%%%%%%%%
% MM beam amplitude
beam_power = zeros(file_size, 4001, MM_narrow_beam_num);%%%%%%%%%%%%

% for different UE speeds
for speed = speeds
    % make dictionary
    mkdir(['ODE_dataset(final_test)_v' num2str(speed)]);
    for i = 1 : file_num
        for j = 1 : file_size
            ...
            while flag == 0
                ...
                direction = rand * 2 * pi;
                a = rand * speed * 0.2;
                % beam tracking duration: 2 s
                % beam training period: 0.01 s
                % beam prediction resolution: 0.001 s
                location = round([initial_x, initial_y] + (speed / 0.2 * [0 : 0.001 : 4]' + ...
                    0.5 * a / 0.2 * ([0 : 0.001 : 4] .^ 2)') * [cos(direction), sin(direction)]);%%%%%%%%%%%%%%%%%%
        ...
        save(['ODE_dataset(final_test)_v' num2str(speed) '\dataset_v' num2str(speed) '_' num2str(i) '.mat'], ...
