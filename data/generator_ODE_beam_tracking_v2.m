...
MM_data = zeros(file_size, 2, 100, MM_narrow_beam_num);
% MM optimal beam index
beam_label = zeros(file_size, 100);
% MM beam amplitude
beam_power = zeros(file_size, 100, MM_narrow_beam_num);

ratios = zeros(file_size, 9, 10);

% for different UE speeds
for speed = speeds
    % make dictionary
    mkdir(['ODE_dataset(v2)_v' num2str(speed)]);
    for i = 1 : file_num
        for j = 1 : file_size
            ...
            while flag == 0
                ...
                % beam prediction resolution: random
                t = zeros(10, 10);
                t(1, :) = [0 : 0.1 : 0.91];
                ratio = sort(rand(9, 10), 1);
                for timestamp = 1 : 10
                    t(2 : end, timestamp) = t(1, timestamp) + 0.1 * ratio(:, timestamp);
                end
                t = reshape(t, 100, 1);
                location = round([initial_x, initial_y] + (speed / 0.2 * t + ...
                    0.5 * a / 0.2 * (t .^ 2)) * [cos(direction), sin(direction)]);
        ...
        MM_data = awgn(MM_data, 104);
         save(['ODE_dataset(v2)_v' num2str(speed) '\dataset_v' num2str(speed) '_' num2str(i) '.mat'], ...
             'MM_data', 'beam_label', 'beam_power', 'ratios');
    end
