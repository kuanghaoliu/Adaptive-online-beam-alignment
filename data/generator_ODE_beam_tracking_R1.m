...
MM_data = zeros(file_size, 2, 1001, MM_narrow_beam_num);
% MM optimal beam index
beam_label = zeros(file_size, 1001);
% MM beam amplitude
beam_power = zeros(file_size, 1001, MM_narrow_beam_num);

% for different UE speeds
for speed = speeds
    % make dictionary
    mkdir(['ODE_dataset(R1_test)_v' num2str(speed)]);
    for i = 1 : file_num
        for j = 1 : file_size
            ...
            while flag == 0
                ...
                % beam prediction resolution: 0.0016 s
                location = round([initial_x, initial_y] + (speed / 0.2 * [0 : 0.001 : 1]' + ...
                    0.5 * a / 0.2 * ([0 : 0.001 : 1] .^ 2)') * [cos(direction), sin(direction)]);
        ...
        save(['ODE_dataset(R1_test)_v' num2str(speed) '\dataset_v' num2str(speed) '_' num2str(i) '.mat'], ...
