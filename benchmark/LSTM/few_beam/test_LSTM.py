...
# 計算正確率
def calculate_acc_BL(predic_beam,out_tensor,labels,ext,P,N,m,BL,beam_power):
    ...
    for i in range(out_shape[0]):  # 每個點
        for j in range(out_shape[1]):  # batch size
            if i % (m + 1) != 0:  # 注意這邊跳過training點
                ...
                predic_beam[j,i] = true_train_index
                # counting accuracy
    ...
    # return P, N
def calculate_acc_BL_eval(predic_beam,out_tensor,labels,ext,P,N,m,BL,beam_power):
    # count predicted instants between two times of beam training  切分的時間點
...
def eval(model, loader,extend_beam_num, total, m):
    ...
    while not done:
        ...
        if count == True:
            ...
            calculate_acc_BL_eval(predict_beam,out_tensor, labels, ext, P, N, m, BL, beam_power)
    ...
    return acur, losses, BL, predict_beam,true_beam # 後面兩個其實沒有用了，當初想看軌跡而已
...
def main():
    ...
    for velocity in [5,10,15,20,25,30]:
        ...
        extend_beam_num = 5
        global change_Strategy  # 設定他為全局變量.
        change_Strategy = True #  訓練跟測試必須使用同一種策略，因為我還沒有要討論低SNR情況，因此我都先使用策略2
        ...
        path2 = r'C:\Users\Aiden\Desktop\my_data\time_01_SNR_104\ODE_dataset(R1_test)_v' + str(velocity)
        ...
        for tt in range(t):
            ...
            acur, losses, BL, predict_beam,true_beam = eval(model, eval_loader, extend_beam_num, total, m)
