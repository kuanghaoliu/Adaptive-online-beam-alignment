...
def calculate_threshold(BL):
    ...
    threshold = np.mean(Beamformining_gain,axis= 0)[-1]
    return threshold
...
def calculate_acc_BL(out_tensor,labels,m,beam_power,seq_len):
    ...
    return acc, BL
def eval(model,model2, loader,batch_size, seq_len, switch_num, extend_beam_num, total, m,initial_switch):
   ...
   # count batch number
    ...
    while not done:  # 有10個檔案，但每個檔案的batch會慢慢吐出來
        ...
        if count == True:
            ...
            # 把剩下的時間序列做完
            for train_number in range(1,channel_total.size()[2]-seq_len+1):  # 決定要從哪一個點開始抓長度
                ...
                if train_number % (switch_num+1) != 0:  # 3個tracking1個training
...
def main():
    ...
    for velocity in [30]:
        ...
        switch_num = 3  # 要做幾次tracking才做一次training
        version_name = 'fix_switch_model_v' + str(velocity) + '_a' + str(velocity * 0.2) + '_seq' + str(seq_len)+'_sw_num'+str(switch_num)
        ...
        for tt in range(t):
            ...
            acur_average, BL, training ,tracking= eval(model,model2, eval_loader,batch_size, seq_len, switch_num, extend_beam_num, total, m,initial_switch)
