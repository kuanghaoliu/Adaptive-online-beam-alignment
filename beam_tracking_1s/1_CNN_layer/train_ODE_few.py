...
from model_ODE_few import Model_3D
import sys
import timeit
...
def get_beam_range_v3(label_increasing,center_beam, num_beams, total_beams):
    # center_beam = 中心beam, num_beams = 要左/右延伸的長度, total_beams
    # Compute the left and right edge indices of the beamforming range
    top = torch.tensor(total_beams - 1)  # 最多就63
    low = torch.tensor(0)  # 最少就0
    total_ = num_beams * 2
    temp = center_beam
    temp2 = center_beam
    index = [center_beam]
    if label_increasing:   # 如果是遞增的
        ...
        # 數看看有幾個
        count_63 = index.count(top)
        count_0 = index.count(low)
        if count_63 > 1 :  # 太靠近右邊
          for e in range(count_63-1) :  # 多個幾個就刪掉幾個
              index.remove(top)
              temp = torch.clamp(temp - 2, 0, total_beams - 1)  # 補在左邊
              index.append(temp)
        elif  count_0 > 1 :  # 太靠近左邊
            for c in range(count_0-1):  # 多個幾個就刪掉幾個
                index.remove(low)
                temp2 = torch.clamp(temp2 + 2, 0, total_beams - 1)  # 補在右邊
                index.append(temp2)

        index.sort(reverse=False)  # 按升序排列

    else:  # 如果是遞減的  63,'5',3,2,1
        right_expand = 1  # 數字大的那邊
        left_expand = total_ -right_expand  # 數字小的那邊
        for e in range(right_expand):
            temp = torch.clamp(temp + 2, 0, total_beams - 1)  # 限制範圍，超過了就在邊邊
            index.append(temp)
        for e in range(left_expand):
        ...
        count_63 = index.count(top)
        count_0 = index.count(low)
        if count_63 > 1:  # 太靠近數字大的那邊
            for e in range(count_63 - 1):  # 多個幾個就刪掉幾個
                index.remove(top)
                temp2 = torch.clamp(temp2 - 2, 0, total_beams - 1)  # 補在數字小的那邊
                index.append(temp2)
        elif count_0 > 1:  # 太靠近數字小的那邊
            for c in range(count_0 - 1):  # 多個幾個就刪掉幾個
                index.remove(low)
                temp = torch.clamp(temp + 2, 0, total_beams - 1)  # 補在數字大的那邊
                index.append(temp)
...
def main():
    ...
    for velocity in [30]:
        ...
        extend_beam_num = 5  # 中心點兩邊展開多少
        global Strategy  # 設定為全局變量來控制使用策略
        Strategy = 2
        version_name = '_ODE_1CNN_1LSTM_60epoch'
        beam_number = min(extend_beam_num*2+1,64)  # 先算出總共會使用多少beam
        info = 'few_v' + str(velocity) + '_a' + str(velocity * 0.2) +'_beam'+str(int(beam_number))+'_Strategy' + str(Strategy)+ version_name
        print("info", info)
        print("device:", device)
        ...
        for tt in range(t):  # 做了5次平均，線會比較平滑一些
            ...
            for e in range(epoch):
                ...
                while not done:
                    ...
                    if Strategy == 1 :
                        ...
                        raise ValueError("Strategy條件不符合要求，引發ValueError異常")
                    # batch_size * 9 * 10
                    ...
                    if count == True:
                        ...
                        if Strategy == 1 or Strategy == 2:
                            calculate_acc_BL(out_tensor, labels, ext, P_, N_,m,BL_, beam_power)
                        elif Strategy == 3:
                            calculate_acc_BL_v2(out_tensor, labels, index, P_, N_,m,BL_, beam_power)
